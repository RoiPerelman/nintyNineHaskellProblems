...
isPrime :: Int -> Bool
isPrime n = not $ or $ map isPrimeHelper [2 .. (n - 1)]
  where
...
goldbach n = goldbachHelper n 2
  where
    goldbachHelper n p =
      let fprime = findNextPrime p
      in if isPrime (n - fprime)
...
goldbachList' start end minVal =
  filter (\(first, second) -> first >= minVal) $ goldbachList start end
